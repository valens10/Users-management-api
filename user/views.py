...
class VerificationsViewset(ViewSet):
    ...
    def verify_account(self, request):
        ...
        if user.email:
            ...
            emails = [user.email]
            send_email_task(emails=emails, subject=subject, message=message)
...
class AuthenticationViewset(ViewSet):
    ...
    def request_verification_code(self, request):
        ...
        if is_username_email(username=username):
            filter_params = {
                "email": username
            }
        ...
        if is_username_email(username):
            ...
            email_message = "<p><b>{code}</b> is your User Management System verification code.</p>".format(code=verification.code)
            
            send_email_task(emails=[username], subject=subject, message=email_message)
